...
public abstract class DomainEventsConsumerBase<T> : ConsumerBase, IDomainEventConsumer<T>, IAsyncDisposable where T : class, IDomainEvent
{
	...
	private void InitChannel()
	{
		...
		Logger.LogInformation($"initializing retry queue '{TopicName}' on exchange '{_mufloneConnectionFactory.ExchangeEventsName}'...");

		_channel.ExchangeDeclare(exchange: _mufloneConnectionFactory.ExchangeEventsName, type: ExchangeType.Topic);
		_channel.QueueDeclare(queue: TopicName,
			durable: true,
			exclusive: false,
			autoDelete: false);
		_channel.QueueBind(queue: $"{_mufloneConnectionFactory.ClientId}.{TopicName}",
			exchange: _mufloneConnectionFactory.ExchangeEventsName,
